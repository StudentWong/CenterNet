...
class Dynamic_conv2d(nn.Module):
    ...
    def set_temperature(self, temp):
        self.attention.set_temperature(temp)
    ...
    def forward(self, x):#将batch视作维度变量，进行组卷积，因为组卷积的权重是不同的，动态卷积的权重也是不同的
        ...
        # print(x.size())
        # print(x.is_contiguous())
        # x = x.view(1, batch_size*in_planes, height, width)
...
if __name__ == '__main__':
    ...
    x = torch.rand(8, 3, 128, 128).cuda()
    model = Dynamic_conv2d(in_planes=3, out_planes=32, kernel_size=3, ratio=0.25, padding=1)
    model = model.cuda()
    y = model(x)
    print(y.shape)
