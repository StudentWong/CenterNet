...
try:
    ...
    pass
try:
    from ..membership import Membership_Activation, Membership_norm, Membership_freeze
except:
    from src.lib.models.membership import Membership_Activation, Membership_norm, Membership_freeze
    pass
...
class DLASeg_freeze_update(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
                 last_level, head_conv, out_channel=0):
        super(DLASeg_freeze_update, self).__init__()
        # print('asd')
        # exit()
        ...
        self.member_c = 0.0 * torch.ones((head_conv, 5), dtype=torch.float, requires_grad=False).cuda()
        self.member_lamda = 1.0 * torch.ones((head_conv, 5), dtype=torch.float, requires_grad=False).cuda()
        self.menber_activation = Membership_freeze()
        # self.menber_activation = Membership_norm(5, 5)
    ...
    def forward(self, x):
        ...
        for head in self.heads:
            ...
            if head == 'hm':
                ...
                z[head], z['norm'] = self.menber_activation(
                    z['ft'].view(origin_shape[0], origin_shape[1], origin_shape[2]*origin_shape[3]),
                    self.member_c, self.member_lamda
                )
                z[head] = z[head].view(origin_shape[0], 5, origin_shape[2], origin_shape[3])
                z['center'] = self.member_c
                z['lambda'] = self.member_lamda
                # print(z['hm'].shape)
                # print(z['norm'].shape)
            else:
...
def get_pose_net_freeze_update(num_layers, heads, head_conv=256, down_ratio=4):
  model = DLASeg_freeze_update('dla{}'.format(num_layers), heads,
                 pretrained=True,
