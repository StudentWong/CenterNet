...
class _centerloss_matrix_function(torch.autograd.Function):
    ...
    def forward(ctx, x, c, gt):
        ...
        # print(type(x))
        assert len(x.shape) == 3 and  len(c.shape) == 2 and len(gt.shape) == 3
        ...
        out = CenterLoss_Forward_Wrapper(x_squeeze, c, gt_squeeze)
        # out = out.clamp(min=min_clip)
        ctx.save_for_backward(x_squeeze.detach(), c.detach(), gt_squeeze.detach(), torch.tensor(N, dtype=torch.int16), torch.tensor(D, dtype=torch.int16), torch.tensor(C, dtype=torch.int16), torch.tensor(wh, dtype=torch.int16))
    ...
    def backward(ctx, grad_output):
        ...
        grad_x = grad_c = grad_gt = None
        x_squeeze, c, gt_squeeze, N, D, C, wh = ctx.saved_variables
        ...
        if ctx.needs_input_grad[0]:
            ...
            CenterLoss_Input_Backward_Wrapper(grad_x, grad_output_squeeze, x_squeeze, c, gt_squeeze)
        ...
        if ctx.needs_input_grad[1]:
            ...
            CenterLoss_Center_Backward_Wrapper(grad_c, grad_output_squeeze, x_squeeze, c, gt_squeeze)
...
class CenterLoss_gt_cuda(nn.Module):
    ...
    # # print(gt_hm.shape)
    # # exit()
    # ret_loss = loss_matrix.sum() / gt_hm.mean(dim=1).sum()
    # # print(ret_loss.shape)
    # return ret_loss
