...
class ModelWithLoss(torch.nn.Module):
  ...
  def backward_model_G(self, batch, optimizer_model_G, lossstat):
    ...
    self.model_gan.set_requires_grad([self.model_gan.netD_A, self.model_gan.netD_B], False)
    optimizer_model_G.zero_grad()

    lambda_idt = self.model_gan.opt.lambda_identity
    lambda_A = self.model_gan.opt.lambda_A
    ...
    if lambda_idt > 0:
      ...
      self.model_gan.idt_B = self.model_gan.netG_B(self.model_gan.real_A)
      self.model_gan.loss_idt_B = self.model_gan.criterionIdt(self.model_gan.idt_B, self.model_gan.real_A) * lambda_A * lambda_idt
    else:
      self.model_gan.loss_idt_A = 0
      self.model_gan.loss_idt_B = 0

    # GAN loss D_A(G_A(A))
    self.model_gan.loss_G_A = self.model_gan.criterionGAN(self.model_gan.netD_A(self.model_gan.fake_B), True)
    # GAN loss D_B(G_B(B))
    self.model_gan.loss_G_B = self.model_gan.criterionGAN(self.model_gan.netD_B(self.model_gan.fake_A), True)
    # Forward cycle loss
    self.model_gan.loss_cycle_A = self.model_gan.criterionCycle(self.model_gan.rec_A, self.model_gan.real_A) * lambda_A
    # Backward cycle loss
    self.model_gan.loss_cycle_B = self.model_gan.criterionCycle(self.model_gan.rec_B, self.model_gan.real_B) * lambda_B
    ...
    self.loss_model_G = self.model_gan.loss_G_A + \
                        self.model_gan.loss_G_B + \
                        self.model_gan.loss_cycle_A + \
                        self.model_gan.loss_cycle_B + \
                        self.model_gan.loss_idt_A + \
                        self.model_gan.loss_idt_B + \
                        loss_fake + loss_true + \
                        self.loss_sement*200

    lossstat['loss_G_A'] = self.model_gan.loss_G_A
    lossstat['loss_G_B'] = self.model_gan.loss_G_B
    lossstat['loss_cycle_A'] = self.model_gan.loss_cycle_A
    lossstat['loss_cycle_B'] = self.model_gan.loss_cycle_B
    lossstat['loss_idt_A'] = self.model_gan.loss_idt_A
    lossstat['loss_idt_B'] = self.model_gan.loss_idt_B
...
class BaseTrainer(object):
  def __init__(
    ...
    self.optimizer_model_G = torch.optim.Adam(itertools.chain(model.parameters(),
                                          gan.netG_A.parameters(),
                                          gan.netG_B.parameters()),
                                         lr=opt.lr, betas=(opt.beta1, 0.999))
    self.optimizer_D = torch.optim.Adam(itertools.chain(gan.netD_A.parameters(), gan.netD_B.parameters()),
                                         lr=opt.lr, betas=(opt.beta1, 0.999))
